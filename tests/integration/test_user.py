...
def test_session_handling(db_session):
    ...
    assert (
        current_count == 1
    ), f"Expected 1 user after adding user1, found {current_count}"
    ...
    assert (
        current_count == 2
    ), f"Should have exactly user1 and user3, found {current_count}"
...
def test_query_methods(db_session, seed_users):
    ...
    assert user_count >= len(
        seed_users
    ), "The user table should have at least the seeded users"
    ...
    assert len(users_by_email) >= len(
        seed_users
    ), "Query should return at least the seeded users"
...
def test_unique_username_constraint(db_session):
    ...
    second_user_data = create_fake_user()
    second_user_data["username"] = first_user_data[
        "username"
    ]  # Force a duplicate username
    second_user = User(**second_user_data)
    db_session.add(second_user)
