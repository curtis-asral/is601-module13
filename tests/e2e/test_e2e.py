...
def test_calculator_add(page, fastapi_server):
    ...
    # Wait for the result element to be populated, then assert the expected value.
    # This avoids race conditions where the JS fetch hasn't completed yet.
    page.wait_for_function("() => document.querySelector('#result') && document.querySelector('#result').innerText.length > 0")
...
def test_calculator_divide_by_zero(page, fastapi_server):
    ...
    # Wait for the result element to be populated, then assert the expected error message.
    page.wait_for_function("() => document.querySelector('#result') && document.querySelector('#result').innerText.length > 0")
